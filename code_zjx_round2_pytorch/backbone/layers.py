...
#
#         for para in list(self.net.parameters()):
...
class SingleNet(nn.Module):
    def __init__(self, modelname, phase,pretrained_path=None, bf16=True):
        ...
        elif self.modelname == "MobileNet_v2":
            self.net = mobilenet_v2(pretrained = True)
            if self.phase == "vertebra":
                self.net.classifier[1] = nn.Linear(in_features=1280, out_features= 2)
            elif self.phase == "disc":
                self.net.classifier[1] = nn.Linear(in_features=1280,out_features= 5)
        else:
            ori_net = None
        ...
        if self.phase == "vertebra":
            self.net.fc = nn.Linear(in_features = self.fc_in_features,out_features = 2)
        elif self.phase == "disc":
            self.net.fc = nn.Linear(in_features= self.fc_in_features,out_features = 5)
        else:
            raise NotImplementedError("phase error")
    ...
    def forward(self,sag_img):

        logit = self.net(sag_img)
    ...
    def load(self,pretrain_path):
        ...
        model_dict = self.net.state_dict()
        print("pre parameters:",model_dict)

        pretrain_dict = {k: v for k, v in pretrain_dict.items() if k in model_dict}
        model_dict.update(pretrain_dict)
        self.net.load_state_dict(model_dict)

        print("after parameters:",pretrain_dict)




class DoubleNet(nn.Module):
    def __init__(self, modelname, phase,pretrained_path=None,bf16=False):
        ...
        if self.modelname == "ResNet18":
            ...
            ori_net = mobilenet_v2(pretrained=True)
        else:
            ori_net = None
            raise NotImplementedError

        if self.modelname == "MobileNet_v2":
            self.fc_in_features = ori_net.classifier[1].in_features
        else:
        ...
        if self.modelname == "ResNeSt18":
            ...
            self.net_sag.avgpool = nn.AdaptiveAvgPool2d(output_size = [1,1])
            self.net_axl.avgpool = nn.AdaptiveAvgPool2d(output_size = [1,1])
        else:
        ...
        if self.phase == "vertebra":
            ...
            self.fc = nn.Linear(self.fc_in_features * 2, config.num_classes[0])


        elif self.phase == "disc":
            self.fc = nn.Linear(self.fc_in_features * 2 ,config.num_classes[1])
