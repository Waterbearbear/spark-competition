...
from backbone.layers import DoubleNet,SingleNet
...
def val(model, weights, dataloader, device):
    ...
    with torch.no_grad():
        ...
        for batch_index, (axial_img, sag_img, target, disc_path, identification, studyID) in enumerate(dataloader):
            # Axial和Sag双路
            axial_img, sag_img,target = axial_img.to(device),sag_img.to(device), target.to(device)
            if model.modelname == "MobileNet_v2":
                axial_img = axial_img.repeat([1,3,1,1])
                sag_img   = sag_img.repeat([1,3,1,1])

            output = model(sag_img,axial_img)
            # 仅用sag
            # sag_img = sag_img.repeat([1, 3, 1, 1]).to(device)
...
def train_axial_model():
    ...
    #### Sag图加 Axial图 数据 #########
    disc_all_dataset = dataset.SagAxialDataset(data_root_path=config.trainPath,
                                                 data_json_path=config.trainjsonPath,
                                                 part = 'disc',
                                                 is_train=True,
                                                 transform=transform.train_transforms())

    vertebra_all_dataset = dataset.SagAxialDataset(data_root_path=config.trainPath,
                                               data_json_path=config.trainjsonPath,
                                               part='vertebra',
                                               is_train=True,
                                               transform=transform.train_transforms())
    ...
    # for parse in ["disc"]:
        ...
        for i in range(config.k_fold):
            ...
            if config.model_type == "Double":
                model = DoubleNet(modelname = modelname,phase = parse,pretrained_path = config.pretrainedPath[0])

            elif config.model_type == "Single":
                model = SingleNet( modelname = modelname ,phase = parse )
            ...
            if config.continue_train == True:
                ...
                pretrained_dict_name = "%s_%s_fold%d_best.pth"%(config.model_name,parse,fold_index)

                pretrain_dict = torch.load(os.path.join(config.models_dir,pretrained_dict_name))
                model_dict = model.state_dict()

                print("before: ",model_dict)
            ...
            for epoch in range(1,config.epochs + 1):
                ...
                for batch_index, (axial_img,sag_img,target,disc_path,identification,studyID) in enumerate(
                    ...
                    #使用ResNeSt时，需要3通道
                    # sag_img,target = sag_img.repeat([1,3,1,1]).to(device),target.to(device)
